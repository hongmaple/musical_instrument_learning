    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['music:recognition:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['music:recognition:edit']"
        >修改</el-button>
      </el-col>
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          :disabled="multiple"-->
<!--          @click="handleDelete"-->
<!--          v-hasPermi="['music:recognition:remove']"-->
<!--        >删除</el-button>-->
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['music:recognition:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="15">
        <el-card style="background-color: rgba(255,255,255,0.9)" class="left-item">
          <el-row type="flex" align="middle" style="flex-wrap: wrap" :gutter="20" v-for="recognition in recognitionList"
                  :key="recognition.id"
                  shadow="never" class="blog-content">
            <div @click="getrecognitionInfo(recognition.id)">
                ...
                <div>
                  ...
                  <div style="margin-bottom: 10px;">
                    <el-button
                      ...
                      type="text"
                      icon="el-icon-download"
                      @click="handleDownload(recognition)"
                    >下载音频</el-button>
                  </div>
                  <div class="blog-info">
                    <div class="user-info">
                      <i class="el-icon-user"></i>
                      <span class="header"> {{ recognition.createBy }}</span>
                    </div>
                    <div class="blog-date">
                      <i class="el-icon-date"></i>
                      <span> {{ recognition.createTime }}</span>
                    </div>
                  </div>
                </div>
              </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="音频地址">
          <fileUpload :limit="1" :file-type="['mp3']" v-model="form.url"/>
        </el-form-item>
      </el-form>
...
<script>
import { listRecognition, getRecognition, delRecognition, addRecognition, updateRecognition } from "@/api/music/recognition";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        url: [
          { required: true, message: "音频地址不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加recognition";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getRecognition(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改recognition";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRecognition(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRecognition(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除recognition编号为"' + ids + '"的数据项？').then(function() {
        return delRecognition(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('music/recognition/export', {
        ...this.queryParams
      }, `recognition_${new Date().getTime()}.xlsx`)
    },
    // 文件下载处理
    handleDownload(row) {
      var name = row.result;
